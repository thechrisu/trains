...
- `TIMER_INTERRUPTS=true` will enable timer interrupts for build targets with `-DE2ETESTING` (_e.g._ `e2etest`, `qemutesting`, and `qemutestingconsole`). The target `e2etest` uses a simulated form of timer interrupts where a task instantly returns from a timer interrupt.
