...
class TestGetcPutc(unittest.TestCase):
    ...
    def test_getcputc_mirror_easy(self):
        ret = real_output = qemu_oneshot_test('test_getcputc_mirror', 'asdq', TIMEOUT, iointerrupts_on=True, timer_interrupts_on=True)
        self.assertEqual(ret, 'asd')

    def test_getcputc_mirror(self):
        ret = real_output = qemu_oneshot_test('test_getcputc_mirror', (expected_getcputc_mirror * 5) + 'q', TIMEOUT, iointerrupts_on=True, timer_interrupts_on=True)
        real_lines = list(filter(lambda x: x != '', real_output.split('\n\r')))
