...
void test_craft_triggers() {
  ...
  location_notification exp[] = {
    { .reason = 2, .subject.loc = { .node = find_node_by_name(&track, "B15"), .offset = find_node_by_name(&track, "B15")->edge[DIR_AHEAD].dist * 100}},
    { .reason = 2, .subject.loc = { .node = find_node_by_name(&track, "BR14"), .offset = find_node_by_name(&track, "BR14")->edge[DIR_CURVED].dist  * 100}},
    { .reason = 2, .subject.loc = { .node = find_node_by_name(&track, "D6"), .offset = find_node_by_name(&track, "D6")->edge[DIR_AHEAD].dist * 100}},
    { .reason = 2, .subject.loc = { .node = find_node_by_name(&track, "D9"), .offset = find_node_by_name(&track, "D9")->edge[DIR_AHEAD].dist * 100}},
    { .reason = 3, .subject.loc = { .node = find_node_by_name(&track, "BR8"), .offset = find_node_by_name(&track, "BR8")->edge[DIR_AHEAD].dist * 100}},
  };
  for (int i = 0; i < n_reqs; i++) {
    logprintf("%s +- %d (%d)\n\r", locs[i].subject.loc.node->name, locs[i].subject.loc.offset, locs[i].reason);
    Assert(locs[i].reason == exp[i].reason);
    Assert(locs[i].subject.loc.node == exp[i].subject.loc.node);
    Assert(locs[i].subject.loc.offset < exp[i].subject.loc.offset);
  }
  Assert(locs[n_reqs - 1].reason == LOCATION_TO_STOP);
