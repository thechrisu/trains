...
void test_collision_prediction() {
  ...
  int max_acc;
  will_collide_with_other_train(0, &me_temp, &temp, 1, turnout_states, &max_acc, 78);
  Assert(max_acc == 14);
  Assert(c.current_speed == 14);
  tmemcpy(&temp, &o, sizeof(temp));
  tmemcpy(&me_temp, &c, sizeof(me_temp));
  will_collide_with_other_train(100000, &me_temp, &temp, 1, turnout_states, &max_acc, 78);
  Assert(c.current_speed == 14);
  // Assert(max_acc > 0 && max_acc < 14);
  tmemcpy(&temp, &o, sizeof(temp));
  tmemcpy(&me_temp, &c, sizeof(me_temp));
  will_collide_with_other_train(100000, &me_temp, &temp, 0, turnout_states, &max_acc, 78);
  Assert(max_acc == 14);
  Assert(c.current_speed ==14);
  ...
  will_collide_with_other_train(200000, &me_temp, &temp, 1, turnout_states, &max_acc, 78);
  // Assert(max_acc == 14);
  Assert(c.current_speed == 14);
