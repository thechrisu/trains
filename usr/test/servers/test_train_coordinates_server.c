...
void test_train_coordinates_server() {
  ...
  send.msg.update_coords.last_sensor.sensor = find_sensor(&track, sensor_offset('B', 15));
  ...
  get_coordinates(train_coords_server_tid, train, &c);
  Assert(c.loc.node == sensor_offset('B', 15));
  ...
  get_coordinates(train_coords_server_tid, train, &c);
  Assert(c.loc.node == sensor_offset('B', 16));
