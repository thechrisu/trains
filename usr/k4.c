...
void user_command_print(int server_tid, user_command *cmd) {
  switch(cmd->type) {
    ...
    case USER_CMD_STOP:
      ...
      break;
    case USER_CMD_TR:
      Printf(server_tid, "%s%sTR %d %d%s", HIDE_CURSOR, CURSOR_ROW_COL(CMD_LINE, 1), cmd->data[0], cmd->data[1], HIDE_CURSOR_TO_EOL);
      break;
    case USER_CMD_SW:
      Printf(server_tid, "%s%sSW %d %c%s", HIDE_CURSOR, CURSOR_ROW_COL(CMD_LINE, 1), cmd->data[0], cmd->data[1], HIDE_CURSOR_TO_EOL);
      break;
    case USER_CMD_RV:
      Printf(server_tid, "%s%sRV %d%s", HIDE_CURSOR, CURSOR_ROW_COL(CMD_LINE, 1), cmd->data[0], HIDE_CURSOR_TO_EOL);
      break;
    case NULL_USER_CMD:
      Printf(server_tid, "%s%sINVALID COMMAND%s", HIDE_CURSOR, CURSOR_ROW_COL(CMD_LINE, 1), HIDE_CURSOR_TO_EOL);
      break;
    default:
      Printf(server_tid, "%s%sUNKNOWN COMMAND%s", HIDE_CURSOR, CURSOR_ROW_COL(CMD_LINE, 1), HIDE_CURSOR_TO_EOL);
  }
}
